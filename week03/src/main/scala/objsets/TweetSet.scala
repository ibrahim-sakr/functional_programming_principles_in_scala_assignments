...
abstract class TweetSet {
  ...
  private def descendingByRetweet0(accumulator: TweetList): TweetList =
    if (this.isEmpty) accumulator
    else remove(this.findMax).descendingByRetweet0(accumulator + this.findMax)
...
class Empty extends TweetSet {
  ...
  override def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("Empty.tweet")

  override def descendingByRetweet: TweetList = throw new java.util.NoSuchElementException("Empty.tweetSet")
}
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    else this.tail.filterAcc(p, accumulate(this.head))
  }
  ...
  override def mostRetweeted: Tweet = mostRetweeted0(this, this.head)

  private def mostRetweeted0(list: TweetSet, accum: Tweet): Tweet = {
    if (list.isEmpty) accum
    else {
      mostRetweeted0(list.tail, compare(list.head, list.tail.head))
    }
  }

  private def compare(current: Tweet, next: Tweet): Tweet =
    if (current.retweets >= next.retweets) current
    else next
}
