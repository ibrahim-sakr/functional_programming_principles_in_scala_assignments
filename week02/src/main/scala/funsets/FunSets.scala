...
object FunSets {
  ...
  /**
    ...
    */
  def forall(s: Set, p: Int => Boolean, accum: Int = -bound): Boolean =
    if (accum > bound) true
    else if (contains(s, accum)) p(accum) && forall(s, p, accum + 1)
    else forall(s, p, accum + 1)

  /**
    * Returns whether there exists a bounded integer within `s`
    * that satisfies `p`.
    */
  def exists(s: Set, p: Int => Boolean, accum: Int = -bound): Boolean = !forall(s, x => !p(x))
